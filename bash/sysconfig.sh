...
function error-message {echo  " ERROR:Can't print text to Sterr">&2
...
function error-exit {"There are errors in the  script and the status is" $?

}
#This function displays help information if the user asks for it on the command line or gives us a bad command line
function displayhelp {echo " Enter --help to get information on correct use of the command"
...
function remove {
  rm /tmp/*.$$
  echo "Temp files are being deleted"
  exit}
